 ...
 *
 * Tests:
 *   1. Tokenization doesn't produce junk Vietnamese syllables
 *   2. IT candidate doesn't get Sales/RE jobs from hard filter
 *   3. Scoring favours skill-matched jobs
 *   4. Tag hygiene — no junk in matchedSkills output
 *   5. Profile extraction works for manual input format
 */

import { removeDiacritics, tokenizeVi, isDisplayableSkill } from "../src/lib/recommend/normalize";
import { hardFilterJobs, isTechCandidate } from "../src/lib/recommend/hardFilter";
...
/* ───────────────── Test 1: Vietnamese tokenization ───────────────── */
...
/* ───────────────── Test 2: IT candidate + hard filter ───────────────── */
...
/* ───────────────── Test 3: IT candidate doesn't get Sales jobs ───────────────── */
console.log("\n── Test 3: Recommendation results for IT candidate ──");
{
  ...
  // Check: none of the recommended jobs should be obviously Sales/RE
  ...
  for (const title of normalizedTitles) {
    for (const kw of salesKeywords) {
      if (title.includes(removeDiacritics(kw))) {
        hasSalesJob = true;
...
console.log("\n── Test 4: Tag hygiene — no junk in skill tags ──");
{
  ...
  const result = recommendJobs("Frontend Developer, React, TypeScript, Node.js, Hồ Chí Minh", getAllJobs(), 6);
