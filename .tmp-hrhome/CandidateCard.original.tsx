    ...
    >
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-semibold text-slate-600">
        <div className="flex items-center gap-1.5 rounded-xl bg-slate-50 px-2.5 py-2">
          ...
          <span className="line-clamp-1">{candidate.location || "Ch╞░a cß║¡p nhß║¡t"}</span>
        </div>
        <div className="flex items-center gap-1.5 rounded-xl bg-slate-50 px-2.5 py-2">
          <span className="material-symbols-outlined text-sm text-slate-400">work_history</span>
          <span className="line-clamp-1">{candidate.experience} n─âm KN</span>
        </div>
      </div>

      <div className="mt-2.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400">
        Cß║¡p nhß║¡t {new Date(candidate.updatedAt).toLocaleDateString("vi-VN")}
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {candidate.skills.length > 0 ? (
          ...
          <span className="text-sm text-slate-500">Ch╞░a c├│ kß╗╣ n─âng c├┤ng khai.</span>
        )}
      </div>
