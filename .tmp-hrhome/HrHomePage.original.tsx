  ...
  useEffect(() => {
    ...
    async function fetchCandidates() {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(payload.error || "Kh├┤ng thß╗â tß║úi danh s├ích ß╗⌐ng vi├¬n.");
        }
        ...
        setError(
          fetchError instanceof Error
            ? fetchError.message
            : "Kh├┤ng thß╗â tß║úi danh s├ích ß╗⌐ng vi├¬n."
        );
      } finally {
        if (active) {
          setLoading(false);
      ...
      <header className="bg-linear-to-b from-blue-50 to-[#f7f9fb] px-4 pt-24 pb-14 sm:px-6 lg:px-8">
        <div className={`${HR_HOME_CONTAINER_CLASS} text-center`}>
          <h1 className="mb-4 text-4xl font-black tracking-tight text-slate-900 md:text-6xl">
            T├¼m kiß║┐m ß╗⌐ng vi├¬n ph├╣ hß╗úp
          </h1>
          <p className="mx-auto mb-10 max-w-3xl text-base font-medium leading-relaxed text-slate-500 md:text-lg">
            Duyß╗çt kho hß╗ô s╞í public theo kß╗╣ n─âng, kinh nghiß╗çm v├á mß╗⌐c ─æß╗Ö sß║╡n s├áng ─æß╗â kß║┐t nß╗æi ─æ├║ng ng╞░ß╗¥i nhanh h╞ín.
          </p>
            ...
            <div className="flex flex-col gap-2 md:flex-row">
              <div className="flex min-w-0 flex-1 items-center gap-3 rounded-2xl px-4">
                ...
                <input
                  ...
                  placeholder="T├¼m ß╗⌐ng vi├¬n theo kß╗╣ n─âng, vß╗ï tr├¡..."
              ...
              <div className="flex min-w-0 flex-1 items-center gap-3 rounded-2xl px-4">
                ...
                <select
                  ...
                  aria-label="Lß╗ìc theo kinh nghiß╗çm"
                  title="Lß╗ìc theo kinh nghiß╗çm"
                  className="h-14 w-full border-none bg-transparent text-base font-semibold text-slate-900 outline-none"
                >
                  <option value="all">Kinh nghiß╗çm bß║Ñt kß╗│</option>
                  <option value="0-1">0-1 n─âm</option>
                  <option value="1-3">1-3 n─âm</option>
                  <option value="3-5">3-5 n─âm</option>
                  <option value="5+">5+ n─âm</option>
                </select>
              </div>
      ...
      <main className="px-4 pb-24 pt-8 sm:px-6 lg:px-8">
        <div className={`${HR_HOME_CONTAINER_CLASS} space-y-8`}>
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-[11px] font-black uppercase tracking-[0.22em] text-slate-400">─Éß╗ü xuß║Ñt cho bß║ín</p>
              <h2 className="mt-2 text-2xl font-black tracking-tight text-slate-950">ß╗¿ng vi├¬n ph├╣ hß╗úp theo nhu cß║ºu tuyß╗ân dß╗Ñng</h2>
            </div>
          </div>

          {recommendations.length === 0 ? (
            <div className="mt-4 rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-5 py-8">
              <p className="text-base font-semibold text-slate-700">Ch╞░a c├│ gß╗úi ├╜ ph├╣ hß╗úp</p>
              <p className="mt-2 text-sm text-slate-500">H├úy ph├ón t├¡ch JD hoß║╖c tß║ío job mß╗¢i ─æß╗â hß╗ç thß╗æng ─æß╗ü xuß║Ñt ß╗⌐ng vi├¬n chuß║⌐n h╞ín.</p>
            ...
            <section className="min-w-0 space-y-5">
              <div className="flex flex-col gap-3 rounded-3xl border border-slate-200 bg-white px-5 py-4 shadow-sm md:flex-row md:items-center md:justify-between">
                <p className="text-sm font-semibold text-slate-500">
                  Hiß╗ân thß╗ï <span className="font-black text-slate-900">{result.total}</span> ß╗⌐ng vi├¬n public
                </p>

                <div className="flex items-center gap-3">
                  <span className="text-sm font-medium text-slate-500">Sß║»p xß║┐p:</span>
                  <select
                    value={filters.sort}
                    onChange={(event) => {
                    ...
                    aria-label="Sß║»p xß║┐p danh s├ích ß╗⌐ng vi├¬n"
                    title="Sß║»p xß║┐p danh s├ích ß╗⌐ng vi├¬n"
                  ...
                  >
                    <option value="latest">Mß╗¢i nhß║Ñt</option>
                    <option value="best_match">Ph├╣ hß╗úp nhß║Ñt</option>
                  </select>
                </div>
              </div>
              ...
              {loading ? (
                ...
                <div className="rounded-[28px] border border-dashed border-slate-300 bg-white px-6 py-14 text-center">
                  <p className="text-xl font-black text-slate-900">Kh├┤ng c├│ ß╗⌐ng vi├¬n ph├╣ hß╗úp</p>
                  <p className="mt-2 text-sm font-medium text-slate-500">
                    H├úy thß╗¡ nß╗¢i lß╗Ång bß╗Ö lß╗ìc hoß║╖c thay ─æß╗òi tß╗½ kh├│a t├¼m kiß║┐m.
                  </p>
                </div>
              ) : (
