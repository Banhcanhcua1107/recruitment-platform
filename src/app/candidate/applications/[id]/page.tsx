    ...
    return (
      <main className="mx-auto max-w-[1360px] space-y-8 px-6 py-10 lg:px-10">
        <nav className="flex items-center gap-3 text-base font-bold text-slate-400">
          ...
          <span className="material-symbols-outlined text-sm">chevron_right</span>
          <span className="font-black tracking-tight text-slate-900">Chi tiết đơn ứng tuyển</span>
        </nav>
        ...
        <section className="rounded-[32px] border border-slate-100 bg-white p-8 shadow-sm lg:p-10">
          <div className="flex flex-col items-start justify-between gap-8 lg:flex-row lg:items-center">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="mb-3 text-lg font-bold italic text-slate-500">
                  {application.job.companyName} • {application.job.location || "Chưa cập nhật địa điểm"}
                </p>
                <div className="flex flex-wrap gap-6 text-base font-black text-slate-400">
                  <span className="flex items-center gap-2">
                    ...
                    Đã nộp: {new Date(application.createdAt).toLocaleDateString("vi-VN")}
                  </span>
            ...
            <div className="flex w-full flex-col gap-4 sm:flex-row lg:w-auto">
              ...
              <a
                href={`mailto:?subject=Đơn ứng tuyển ${application.job.title}`}
            ...
            <div className="p-10 lg:p-14">
              <div className="flex flex-col gap-0">
                {application.events.length === 0 ? (
                  <p className="text-slate-500">Chưa có lịch sử xử lý cho đơn ứng tuyển này.</p>
                ) : (
                    ...
                    <TimelineStep
                      ...
                      description={
                        typeof event.metadata.status === "string"
                          ...
                          : "Đã ghi nhận sự kiện xử lý trong hệ thống."
                      }
          ...
          <aside className="flex flex-col gap-8 lg:col-span-4">
            <div className="rounded-[32px] border border-slate-100 bg-white p-8 shadow-sm">
              ...
              <div className="space-y-5">
                <SummaryItem icon="business" label="Công ty" value={application.job.companyName} />
                <SummaryItem icon="work" label="Hình thức" value={application.job.employmentType} />
                <SummaryItem icon="badge" label="Cấp bậc" value={application.job.level} />
                <SummaryItem icon="pin_drop" label="Địa điểm" value={application.job.fullAddress || application.job.location} />
              </div>
            </div>
            ...
            <div className="rounded-[32px] border border-slate-100 bg-white p-8 shadow-sm">
              ...
              <p className="whitespace-pre-wrap text-base leading-relaxed text-slate-600">
                {application.coverLetter || "Bạn chưa gửi thư giới thiệu cho đơn ứng tuyển này."}
              </p>
            </div>
          </aside>
        </div>
