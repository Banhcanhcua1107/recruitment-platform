...
export function UploadCard({ onClick, compact = false }: UploadCardProps) {
  return (
    ...
    >
      <div
        className={`mb-3 flex items-center justify-center rounded-full bg-sky-50 transition-colors duration-200 group-hover:bg-sky-100 ${
      ...
      <p
        className={`mt-1 max-w-40 text-center ${compact ? "text-[11px]" : "text-sm"} font-medium text-slate-500`}
      >
        Nhap nhanh noi dung tu CV hien co
      </p>

      <div
        className={`mt-3 flex items-center gap-1.5 rounded-full border border-sky-200 bg-sky-50 transition-colors duration-200 group-hover:border-sky-300 group-hover:bg-sky-100 ${
          compact ? "px-2.5 py-1" : "px-3.5 py-1.5"
        }`}
      >
        <ScanLine size={compact ? 12 : 13} className="text-sky-700" />
        <span
          className={`${compact ? "text-[9px]" : "text-[11px]"} font-bold uppercase tracking-[0.08em] text-sky-700`}
        >
          PDF JPG PNG DOCX
        </span>
      </div>
