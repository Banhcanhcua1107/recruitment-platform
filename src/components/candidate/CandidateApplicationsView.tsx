...
import type { CandidateApplicationListItem } from "@/types/candidate-dashboard";
...
export function CandidateApplicationsView({
  ...
  const [filter, setFilter] = useState<"all" | RecruitmentPipelineStatus>("all");
      ...
      {filteredApplications.length > itemsPerPage ? (
        <div className="flex flex-col items-center justify-between gap-6 pt-4 sm:flex-row">
          <p className="text-lg font-bold italic text-slate-400">
            Hiển thị{" "}
            <span className="font-black text-slate-900">{paginatedApplications.length}</span> trên
            tổng số{" "}
            <span className="font-black text-slate-900">{filteredApplications.length}</span> đơn
          </p>
...
function ApplicationRow({
  ...
  return (
    <tr className="group transition-colors hover:bg-slate-50/80">
      ...
      <td className="px-8 py-6">
        <div className="flex items-center gap-4">
          {application.job.logoUrl ? (
            <div className="relative size-11 overflow-hidden rounded-xl border border-slate-200">
              <Image
      ...
      <td className="px-8 py-6">
        <span
          className={`inline-flex rounded-xl border px-4 py-1.5 text-xs font-black uppercase tracking-widest ${badge.className}`}
        >
          {badge.label}
        </span>
      </td>
  ...
  return (
    <button
      ...
      className={[
        ...
        disabled
          ? "cursor-not-allowed opacity-50 hover:border-slate-200 hover:text-slate-400"
          : "cursor-pointer",
      ].join(" ")}
    >
